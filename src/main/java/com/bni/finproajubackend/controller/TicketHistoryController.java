...
public class TicketHistoryController {
    ...
    public ResponseEntity<TemplateResponseDTO<List<TrackTicketStatusResponseDTO>>> getMyTicketHistoryDetail(Authentication authentication, @PathVariable Long id){
        try{
            List<TrackTicketStatusResponseDTO> responseDTOList = ticketHistoryStatusService.trackMyTicketStatus(authentication, id);
            return ResponseEntity.ok(responseService.apiSuccess(responseDTOList, "Success"));
        }
        catch (Exception e){
            errorDetails.put("message", "Ticket not found: " + e.getMessage());
